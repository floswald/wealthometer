	...
	var Bar = function(data,config,ctx){
		...
		function drawBars(animPc){
			...
			for (var i=0; i<data.datasets.length; i++){
				...
				for (var j=0; j<data.datasets[i].data.length; j++){
					...
					if ( j == data.datasets[i].highLightPos ) {
						ctx.fillStyle = data.datasets[i].highLightColor;
						// resetColor = true;
						ctx.beginPath();
						ctx.moveTo(barOffset, xAxisPosY - zeroY);
						ctx.lineTo(barOffset, xAxisPosY);
					...
					if ( j == data.datasets[i].HLP2 ) {
						ctx.fillStyle = data.datasets[i].HLC2;
						// resetColor = true;
						ctx.beginPath();
						ctx.moveTo(barOffset, xAxisPosY - zeroY);
						ctx.lineTo(barOffset, xAxisPosY);
		...
		function drawScale(){
			...
			for (var i=0; i<data.labels.length; i++){
				ctx.save();
				if (rotateLabels > 0){
					ctx.translate(yAxisPosX + i*valueHop - 5,xAxisPosY + config.scaleFontSize);
